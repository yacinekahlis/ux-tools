...
interface ToolPageProps {
  // NOTE: tool ici est "safe" (sans calculate) venant du server
  tool: ToolWithSEO
}
...
export function ToolPage({ tool }: ToolPageProps) {
  ...
  // ✅ IMPORTANT: on ne lit plus tool.calculate (car tool est "safe" sans fonctions)
  // On récupère le tool complet (avec calculate) via le registry par slug
  const result: ToolResult = useMemo(() => {
    const fullTool = getToolBySlug(tool.slug)
    if (fullTool?.calculate) {
      return fullTool.calculate(inputs, uncheckedIds, tool as any)
    }
    return { score: 0, label: "Error", color: "text-danger", fixes: [] }
  }, [inputs, uncheckedIds, tool.slug, tool])
                  ...
                  <div className="space-y-3">
                    {tool.checklist.map((item) => {
                      ...
                      return (
                          <label
                              key={item.id}
                              className={`flex items-start gap-4 p-4 rounded-[8px] border transition-all cursor-pointer group select-none ${
                                  isChecked ? "bg-card border-borderHover" : "bg-card border-border hover:border-borderHover"
                              }`}
                          >
                            <div
                                className={`mt-0.5 flex-shrink-0 w-4 h-4 rounded-[3px] border flex items-center justify-center transition-all ${
                                    isChecked
                                        ? "bg-textMain border-textMain animate-check"
                                        : "border-border group-hover:border-borderHover bg-transparent"
                                }`}
                            >
                              {isChecked && (
                            ...
                            <div>
                              <div
                                  className={`text-sm font-medium transition-colors ${
                                      isChecked ? "text-textMain" : "text-textSecondary"
                                  }`}
                              >
                                {item.text}
                              </div>
          ...
          <div className="space-y-6">
            <Card className="p-6 sticky top-24 shadow-xl shadow-black/40">
              <div className="text-center mb-6 pt-2">
                ...
                <div
                    key={scoreAnimKey}
                    className={`text-5xl font-bold mb-2 tracking-tight ${result.color} ${
                        scoreAnimKey > 0 ? "animate-score" : ""
                    }`}
                >
                  {result.score}
                </div>
